...
public_users.get('/', async (req, res) => {
    try {
        const response = await axios.get('/api/books');   // relative URL – no port!
...
public_users.get('/isbn/:isbn', async (req, res) => {
    try {
        const isbn = req.params.isbn;
        const response = await axios.get(`/api/books/isbn/${encodeURIComponent(isbn)}`);
        return res.json(response.data);
    } catch (error) {
...
public_users.get('/author/:author', async (req, res) => {
    try {
        const author = req.params.author;
        const response = await axios.get(`/api/books/author/${encodeURIComponent(author)}`);
        return res.json(response.data);
    } catch (error) {
        if (error.response && error.response.status === 404) {
...
public_users.get('/title/:title', async (req, res) => {
    try {
        const title = req.params.title;
        const response = await axios.get(`/api/books/title/${encodeURIComponent(title)}`);
        return res.json(response.data);
    } catch (error) {
        if (error.response && error.response.status === 404) {
